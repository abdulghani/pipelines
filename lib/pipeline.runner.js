...
class PipelineRunner {
    ...
    MonitorDeployment(webApi, release) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            else if (res.environments.find((item) => failedStatuses.includes(item.status))) {
                console.log("FAILURE DEPLOYMENT", res.environments.map((item) => releaseStatus[item.status]));
                process.exit(1);
            }
        });
    }
